...
class CustomKTOTrainer:
    ...
    def convert_dataset(cls, raw_datasets: list[RawDataset]) -> Dataset:
        ...
        for raw_dataset in raw_datasets:
            for row in raw_dataset.get_data(split=SplitType.TRAIN):
                if row.chosen:
                    rows.append(
                        {"prompt": row.prompt, "completion": row.chosen, "label": True, "preference": row.preference}
                    )
                else:
                    rows.append(
    ...
    def get_trainer(
        ...
        Returns:
            kto_trainer: One can call kto_trainer.train() to then run the training loop.
        """
        ...
        train_dataset = CustomKTOTrainer.convert_dataset(raw_datasets=raw_datasets)
