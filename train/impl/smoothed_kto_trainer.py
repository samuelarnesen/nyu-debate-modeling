## NOTE: This is an edited version of Huggingface's KTO implementation that uses custom label smoothing
...
class SmoothedKTOTrainer(Trainer):
    r"""
    Initialize SmoothedKTOTrainer.
    ...
    def kto_loss(
        ...
        if policy_chosen_logps.shape[0] != 0 or reference_chosen_logps.shape[0] != 0:
            """correct_preferences = (correct_preferences * (1 - self.correct_baseline)) / (
                (correct_preferences * (1 - self.correct_baseline)) + (self.correct_baseline * (1 - correct_preferences))
            )"""
            correct_preferences = correct_preferences / (correct_preferences + self.correct_baseline)
            correct_label_smoothing = (1 - correct_preferences).to("cuda")
            pref = correct_label_smoothing.item()
            self.logger.info(f"New correct label smoothing is {pref}")

            chosen_logratios = policy_chosen_logps - reference_chosen_logps
            chosen_losses = ((1 - F.sigmoid(self.beta * (chosen_logratios - kl))) * (1 - correct_label_smoothing)) + (
                (1 - F.sigmoid(self.beta * (kl - chosen_logratios))) * correct_label_smoothing
            )
        ...
        if policy_rejected_logps.shape[0] != 0 or reference_rejected_logps.shape[0] != 0:
            """incorrect_preferences = (incorrect_preferences * (1 - self.incorrect_baseline)) / (
                (incorrect_preferences * (1 - self.incorrect_baseline))
                + (self.incorrect_baseline * (1 - incorrect_preferences))
            )"""
            incorrect_preferences = incorrect_preferences / (incorrect_preferences + self.incorrect_baseline)
            incorrect_label_smoothing = (1 - incorrect_preferences).to("cuda")
            pref = incorrect_label_smoothing.item()
            self.logger.info(f"New incorrect label smoothing is {pref}")

            rejected_logratios = policy_rejected_logps - reference_rejected_logps
            rejected_losses = ((1 - F.sigmoid(self.beta * (rejected_logratios - kl))) * (1 - incorrect_label_smoothing)) + (
                (1 - F.sigmoid(self.beta * (kl - rejected_logratios))) * incorrect_label_smoothing
            )
    ...
    def get_batch_loss_metrics(
        ...
        if "reference_logps" in batch:
            ...
            with torch.no_grad():
                if self.ref_model is None:
                    with self.null_ref_context():
                        (reference_chosen_logps, reference_rejected_logps, _, _, reference_KL_logps, _, _) = self.forward(
                            self.model, batch
                        )
                else:
                    (reference_chosen_logps, reference_rejected_logps, _, _, reference_KL_logps, _, _) = self.forward(
