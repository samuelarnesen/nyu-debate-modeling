## NOTE: This is an edited version of Huggingface's KTO implementation, but converted to use implement "Binary Classifier Optimization for Large Language Model Alignment"
...
class BCOTrainer(Trainer):
    ...
    def __init__(
        ...
        if args.ref_model_init_kwargs is None:
            ...
            raise ValueError("You passed ref_model_kwargs to the KTOTrainer. But your ref_model is already instantiated.")
        else:
        ...
        if isinstance(ref_model, str):
            warnings.warn(
                "You passed a ref model_id to the KTOTrainer. This will automatically create an " "`AutoModelForCausalLM`"
            )
        ...
        if tokenizer is None:
            raise ValueError("max_length or a tokenizer must be specified when using the default DPODataCollatorWithPadding")
        ...
        self.correct_baseline = 0.62
        self.incorrect_baseline = 0.27

        self.m = 32
        self.correct_ps_history = [self.correct_baseline for i in range(self.m)]
        self.incorrect_ps_history = [self.incorrect_baseline for i in range(self.m)]
        self.correct_logratio_history = [0 for i in range(self.m)]
        self.incorrect_logratio_history = [0 for i in range(self.m)]
    ...
    def forward(
        ...
        prefs = torch.FloatTensor(batch.get("preference"))
        correct_preferences = prefs[chosen_idx]
        incorrect_preferences = prefs[rejected_idx]
    ...
    def bco_loss(
        ...
        correct_preferences: torch.FloatTensor,
        incorrect_preferences: torch.FloatTensor,
    ) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        ...
        if policy_chosen_logps.shape[0] != 0 or reference_chosen_logps.shape[0] != 0:
            p_baseline = sum(self.correct_ps_history[-self.m :]) / self.m
            p = ((correct_preferences / (correct_preferences + p_baseline))).to("cuda")

            chosen_logratios = policy_chosen_logps - reference_chosen_logps
            logratio_baseline = sum(self.correct_logratio_history[-self.m :]) / self.m
            y = torch.clip(F.sigmoid(self.beta * (chosen_logratios - logratio_baseline)), lower_clip, upper_clip)

            chosen_losses = -((p * torch.log2(y)) + ((1 - p) * torch.log2(1 - y)))
            self.logger.info(
                f"overall: {chosen_losses.item()}, p: {p.item()}, y: {y.item()}, p_baseline: {p_baseline}, logratio_baseline: {logratio_baseline}"
            )

            chosen_rewards = self.beta * chosen_logratios.detach()
            self.correct_ps_history.extend([elem.detach().cpu().item() for elem in correct_preferences])
            self.correct_logratio_history.extend([elem.detach().cpu().item() for elem in chosen_logratios])
        else:
        ...
        if policy_rejected_logps.shape[0] != 0 or reference_rejected_logps.shape[0] != 0:
            p_baseline = sum(self.incorrect_ps_history[-self.m :]) / self.m
            p = ((incorrect_preferences / (incorrect_preferences + p_baseline))).to("cuda")

            rejected_logratios = policy_rejected_logps - reference_rejected_logps
            logratio_baseline = sum(self.incorrect_logratio_history[-self.m :]) / self.m
            y = torch.clip(F.sigmoid(self.beta * (rejected_logratios - logratio_baseline)), lower_clip, upper_clip)

            rejected_losses = -((p * torch.log2(y)) + ((1 - p) * torch.log2(1 - y)))
            self.logger.info(
                f"overall: {rejected_losses.item()}, p: {p.item()}, y: {y.item()}, p_baseline: {p_baseline}, logratio_baseline: {logratio_baseline}"
            )

            rejected_rewards = self.beta * rejected_logratios.detach()
            self.incorrect_ps_history.extend([elem.detach().cpu().item() for elem in incorrect_preferences])
            self.incorrect_logratio_history.extend([elem.detach().cpu().item() for elem in rejected_logratios])
        else:
            # lists can't be empty -- if they are, then accelerate.gather will hang
    ...
    def get_batch_loss_metrics(
        ...
        if "reference_logps" in batch:
            ...
            with torch.no_grad():
                if self.ref_model is None:
                    with self.null_ref_context():
                        (reference_chosen_logps, reference_rejected_logps, _, _, _, _) = self.forward(self.model, batch)
                else:
                    (reference_chosen_logps, reference_rejected_logps, _, _, _, _) = self.forward(self.ref_model, batch)

        losses, chosen_rewards, rejected_rewards = self.bco_loss(
            policy_chosen_logps,
            policy_rejected_logps,
